# Codex / OpenAI Agents — Global Instructions

> Note: OpenAI Codex CLI reads AGENTS.md from the current project directory.
> A global version is not officially supported yet — place this file in each
> project root as needed. This file is kept here as a template.
> Copy to a project: `cp ~/.dotfiles/ai/codex/AGENTS.md ~/dev/my-project/AGENTS.md`
...
- **Continuous Sync**: If we pivot during implementation, update the plan immediately.
- **Zero-Drift**: When modifying code, update the corresponding documentation in the same step.
...
- **No fallbacks or symptom-masking** unless explicitly asked.
...
- No planning or analysis documents unless asked.
...
- Flag look-ahead bias with inline comment: `# LOOK-AHEAD RISK: ...`
- Point-in-time discipline: data available at T used only at T.
- Vectorised operations over row-level loops.
...
data/raw/        never modified after ingestion
